...
void test_entry(char *nume_fisier, int raspuns) {
    ...
    /*for(int i = 0; i < lista_intervale2D->dimensiune; i++) {
        Punct2D *right_point = lista_intervale2D->intervale[i]->punct_dreapta;
        Punct2D *left_point = lista_intervale2D->intervale[i]->punct_stanga;

        printf("%d %d", left_point->x, left_point->y);
        printf(" %d %d\n", right_point->x, right_point->y);
        printf("%d\n", is_horizontal(lista_intervale2D->intervale[i]));
    }
    printf("%d\n", lista_intervale2D->dimensiune);*/
