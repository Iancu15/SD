...
// calculeaza numarul de puncte dintre 2 intervale care sunt pe aceeasi dreapta
// se specifica in parametru daca ambele intervale sunt paralele cu axa x sau y
int weighted_diff(Interval2D *a, Interval2D *b, char axis) {
    int minim, maxim;

    if (!(ccmp(axis, 'y'))) {
        minim = min(a->punct_dreapta->x, b->punct_dreapta->x);
        maxim = max(a->punct_stanga->x, b->punct_stanga->x);

        // daca e negativ rezultatul inseamna ca nu au niciun punct comun
        if (minim >= maxim) {
            return minim - maxim + 1;
        }
    } else {
        minim = min(a->punct_dreapta->y, b->punct_dreapta->y);
        maxim = max(a->punct_stanga->y, b->punct_stanga->y);
    ...
    // returnez 0 in caz ca nu se verifica if-ul(caz-ul rezultatului negativ)
    return 0;
}
...
int calculeaza_numar_intersectii_trivial(ListaIntervale2D *lista) {
    ...
    for(i = 0; i < lista->dimensiune; i++) {
        ...
        for(j = i + 1; j < lista->dimensiune; j++) {
            ...
            // primele 2 if-uri nu sunt necesare pentru derularea testelor
            if (is_i_horizontal && is_j_horizontal) {
                // daca sunt pe aceeasi dreapta calculez intersectia de puncte
                if (l[i]->punct_dreapta->y == l[j]->punct_dreapta->y) {
                    intersection_num += weighted_diff(l[i], l[j], 'y');
                }

            } else if (!is_i_horizontal && !is_j_horizontal) {
                if (l[i]->punct_dreapta->x == l[j]->punct_dreapta->x) {
                    intersection_num += weighted_diff(l[i], l[j], 'x');
                }
